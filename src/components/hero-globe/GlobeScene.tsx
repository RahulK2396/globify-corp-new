  ...
  useFrame(() => {
    if (glowRef.current) {
      glowRef.current.scale.setScalar(
        1 + Math.sin(Date.now() * 0.003 + delay) * 0.4,
      );
...
const FuturisticGlobe = ({
  ...
  const hitSphere = useMemo(
    () => new THREE.Sphere(new THREE.Vector3(0, 0, 0), R + 0.3),
    [],
  );

  const isPointerOnGlobe = useCallback(
    (e: PointerEvent) => {
  ...
  const arcs = useMemo(() => {
    ...
    return connections.map(([fi, ti], i) => {
      ...
      const mid = start
        ...
        .multiplyScalar(R * h);
      return {
        points: new THREE.QuadraticBezierCurve3(start, mid, end).getPoints(80),
        delay: i * 0.2,
      };
    });
  }, [cityLocations]);
  ...
  useFrame((_, delta) => {
    ...
    if (atmosphereRef.current)
      atmosphereRef.current.scale.setScalar(
        1 + Math.sin(Date.now() * 0.0008) * 0.015,
      );
    if (outerGlowRef.current)
      outerGlowRef.current.scale.setScalar(
        1 + Math.sin(Date.now() * 0.0006 + 1) * 0.01,
      );
    if (rimRef.current)
