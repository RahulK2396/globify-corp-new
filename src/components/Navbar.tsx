...
if (typeof window !== "undefined") {
  const preload = () => {
    [megaMenuServices, megaMenuIndustries, megaMenuProducts].forEach((src) => {
      const img = new Image();
      img.src = src;
    });
  };
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-hero/90 backdrop-blur-md border-b border-hero-foreground/10">
      <div className="container mx-auto px-6 py-3 flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-3">
          ...
          >
            <img src={whatsappIcon} alt="WhatsApp" className="w-full h-full object-cover" width={36} height={36} />
          </a>
          <button
            ...
            <div className="w-[calc(100%-3rem)] max-w-[1440px] h-[360px] bg-background rounded-2xl shadow-2xl shadow-foreground/10 border border-border/20 px-12 py-10 overflow-hidden">
              {activeMega === "services" && (
                <div className="flex gap-10 h-full">
                  ...
                  >
                    <img src={megaMenuServices} alt="" className="absolute inset-0 w-full h-full object-cover" />
                  ...
                  <div className="flex-1 grid grid-cols-3 gap-x-12">
                    <div>
                      ...
                      {servicesMenu.slice(0, 2).map((item) => (
                        <Link key={item.title} to={item.href} className="block mb-6 group">
                      ...
                      <Link to="/erp-solutions" className="block mb-6 group">
                      ...
                      <Link to="/hire-developers" className="block mb-6 group">
                    ...
                    <div>
                      ...
                      {/* AI & Automation */}
                      <Link to="/ai-automation" className="block mb-6 group">
                      ...
                      {/* Digital Marketing */}
                      <Link to="/digital-marketing" className="block mb-3 group">
                      ...
                      {marketingServices.map((item) => (
                        <Link key={item.title} to={item.href} className="block mb-2 group">
                    ...
                    <div>
                      ...
                      {/* E-Commerce */}
                      <Link to="/ecommerce" className="block mb-3 group">
                      ...
                      <Link to="/ecommerce" className="inline-flex items-center gap-1 text-sm font-semibold text-primary hover:underline mt-1">
                        View All Platforms <span aria-hidden="true">→</span>
                      </Link>
                      <Link to="/hire-developers" className="mt-4 inline-flex items-center gap-2 bg-foreground text-background px-4 py-2 rounded-md text-sm font-semibold hover:bg-foreground/90 transition-colors">
              ...
              {activeMega === "industries" && (
                <div className="flex gap-10 h-full">
                  <Link
                    ...
                    className="group flex-shrink-0 w-[280px] rounded-[20px] relative overflow-hidden flex flex-col justify-end"
                  >
                    <img src={megaMenuIndustries} alt="" className="absolute inset-0 w-full h-full object-cover" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent" />
                    <div className="relative z-10 p-7 pt-32">
                  ...
                  <div className="flex-1 grid grid-cols-3 gap-x-12">
                    ...
                    <div>
                      ...
                      <Link to="/hire-developers" className="inline-flex items-center gap-2 bg-foreground text-background px-4 py-2 rounded-md text-sm font-semibold hover:bg-foreground/90 transition-colors">
                        <Users className="w-4 h-4" /> Hire a Developer
                      </Link>
                    </div>
                  </div>
              ...
              {activeMega === "products" && (
                <div className="flex gap-10 h-full">
                  <Link
                    ...
                    className="group flex-shrink-0 w-[280px] rounded-[20px] relative overflow-hidden flex flex-col justify-end"
                  >
                    <img src={megaMenuProducts} alt="" className="absolute inset-0 w-full h-full object-cover" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent" />
                    <div className="relative z-10 p-7 pt-32">
                  ...
                  <div className="flex-1 flex flex-col gap-6">
                    <div className="grid grid-cols-2 gap-x-12">
                      {productsMenu.map((item) => (
                        <Link key={item.title} to={item.href} className="block group p-5 rounded-xl border border-border/40 hover:border-primary/20 hover:bg-muted/5 transition-all">
                    ...
                    <Link to="/hire-developers" className="inline-flex items-center gap-2 bg-foreground text-background px-4 py-2 rounded-md text-sm font-semibold hover:bg-foreground/90 transition-colors w-fit">
                      <Users className="w-4 h-4" /> Hire a Developer
                    </Link>
                  </div>
                </div>
        ...
        {isOpen && (
          ...
          >
            <div className="p-6 pt-24 space-y-1">
              <Link to="/" onClick={() => setIsOpen(false)} className="block py-3.5 text-[15px] font-medium text-hero-foreground/80 hover:text-primary border-b border-hero-foreground/[0.06]">
                Home
              </Link>
              <Link to="/about" onClick={() => setIsOpen(false)} className="block py-3.5 text-[15px] font-medium text-hero-foreground/80 hover:text-primary border-b border-hero-foreground/[0.06]">
                About Us
              </Link>
              ...
              <Link to="/projects" onClick={() => setIsOpen(false)} className="block py-3.5 text-[15px] font-medium text-hero-foreground/80 hover:text-primary border-b border-hero-foreground/[0.06]">
                Our Projects
              </Link>
              <Link to="/resources" onClick={() => setIsOpen(false)} className="block py-3.5 text-[15px] font-medium text-hero-foreground/80 hover:text-primary border-b border-hero-foreground/[0.06]">
                Resources
              </Link>
              <Link to="/blog" onClick={() => setIsOpen(false)} className="block py-3.5 text-[15px] font-medium text-hero-foreground/80 hover:text-primary border-b border-hero-foreground/[0.06]">
                Blog
              </Link>
